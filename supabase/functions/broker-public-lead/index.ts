// Supabase Edge Function: broker-public-lead
//
...
// создаёт событие и при наличии секретов отправляет уведомление в Telegram.
//
...
// ALLOW_ORIGINLESS      default false
// RATE_LIMIT_PER_HOUR   default 8
// MIN_FILL_MS           default 1500
...
async function findExistingLead(requestId: string): Promise<JsonRecord | null> {
  ...
  const { data, error } = await supabase
    .from('broker_leads')
    .select('id, request_id, status, technical_priority, qualification')
...
function telegramSummary(row: JsonRecord): string {
  const qualification = asRecord(row.qualification);
  const tracking = asRecord(row.tracking);
  const current = asRecord(tracking.current);
  return [
    ...
    `Телефон: ${row.phone || ''}`,
    `Город: ${row.city || ''}`,
    `Связь: ${row.preferred_contact || ''}`,
    `Задача: ${row.scenario || ''}`,
    `Объект: ${row.object_type || ''}`,
    `Стоимость: ${row.object_price_text || ''}`,
    `Взнос: ${row.down_payment_text || ''}`,
    `Доход: ${row.income_type || ''}`,
    `Приоритет: ${qualification.status || ''}, ${qualification.score || 0} баллов`,
    `Источник: ${current.utm_source || ''} / ${current.utm_medium || ''} / ${current.utm_campaign || ''}`,
    `Страница: ${row.source_page || row.page_url || ''}`
  ].join('\n');
}

async function notifyTelegram(text: string): Promise<'sent' | 'disabled'> {
...
Deno.serve(async (request) => {
  ...
  if (!isAllowedOrigin(origin)) {
    return jsonResponse({ ok: false, success: false, error: 'origin_not_allowed' }, 403, origin);
  }
  if (request.method !== 'POST') {
    return jsonResponse({ ok: false, success: false, error: 'method_not_allowed' }, 405, origin);
  }
  if (!supabase) {
    return jsonResponse({ ok: false, success: false, error: 'server_not_configured' }, 503, origin);
  }
  ...
  await addEvent(
    String(data.id),
    validation.lead.requestId,
    'created',
    'Заявка создана с сайта',
    'Заявка принята через Supabase Edge Function v2',
    {
  ...
  try {
    notificationStatus = await notifyTelegram(telegramSummary(row));
  } catch (error) {
    notificationStatus = 'failed';
    console.error('broker_lead_telegram_failed', error instanceof Error ? error.message : 'unknown');
  }

  if (notificationStatus !== 'disabled') {
    await supabase.from('broker_leads').update({ notification_status: notificationStatus }).eq('id', data.id);
    await addEvent(
      String(data.id),
      validation.lead.requestId,
      notificationStatus === 'sent' ? 'notification_sent' : 'notification_failed',
      notificationStatus === 'sent' ? 'Telegram-уведомление отправлено' : 'Ошибка Telegram-уведомления',
      notificationStatus === 'sent' ? 'Ответственный канал уведомлён' : 'Заявка сохранена, но уведомление требует проверки'
    );
  }
