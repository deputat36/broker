...
Ручной запрос может выполнить только переход:

```text
failed → pending → sending → sent | failed
```

Запрещены ручные переходы из:
...
Зависшее `sending` восстанавливается штатным атомарным claim из пятой миграции, а не административным retry.
...
6. проверки очереди без персональных данных;
7. фиксации ответственного и процедуры отката в issue №7.
