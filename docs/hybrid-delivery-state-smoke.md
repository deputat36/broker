...
## 2. Порядок браузерных скриптов

На `/online-zayavka/` проверить порядок:

1. `application-delivery-keepalive.js`;
2. `application-inputs.js`;
3. `application-preparation.js`;
4. основной `online-application.js` из layout.

Только запрос `delivery_receipt + both` должен получать `keepalive: true`. Основная заявка и Web3Forms не должны получать этот флаг автоматически.
...
В DevTools подтвердить, что receipt-запрос имеет `keepalive` и может завершиться после начала перехода на `/spasibo/`.

## 6. Медленный Supabase
...
## 8. Restricted, hold и anonymized
