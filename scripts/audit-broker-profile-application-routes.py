#!/usr/bin/env python3
"""Проверяет прямые маршруты со страницы «О брокере» в онлайн-заявку."""
...
def validate_built(path: Path) -> int:
    ...
    for anchor in anchors:
        ...
        if parsed.path == APPLICATION_URL and "source" in query and "scenario" in query:
            ...
            phone_links += 1
        elif parsed.path == "/uslugi/":
            services_links += 1
        elif parsed.path == "/kontakty/":
            contacts_links += 1
    ...
    if consultation_links < 1:
        fail(path, "Должна сохраниться отдельная ссылка на бесплатную консультацию")
        errors += 1
    if phone_links < 2:
        fail(path, f"Должны сохраниться минимум две телефонные ссылки, найдено: {phone_links}")
        errors += 1
    if services_links < 1:
        fail(path, "Должна сохраниться ссылка на каталог услуг")
        errors += 1
    if contacts_links < 1:
        fail(path, "Должна сохраниться ссылка на страницу контактов")
        errors += 1
...
def main() -> int:
    ...
    built_file = site_dir / "o-brokere" / "index.html"
    errors = 0

    if not SOURCE_FILE.is_file():
        fail(SOURCE_FILE, "Исходная страница о брокере отсутствует")
        errors += 1
    else:
        errors += validate_source(SOURCE_FILE)

    if not built_file.is_file():
        fail(built_file, "Собранная страница о брокере отсутствует")
        errors += 1
    else:
        errors += validate_built(built_file)

    if errors:
        print(f"Аудит прямых заявок со страницы о брокере завершён с ошибками: {errors}")
        return 1

    print(
        "Маршруты страницы о брокере подтверждены: два прямых CTA, "
        "источник /o-brokere/, единый сценарий и сохранённые контакты"
    )
    return 0
