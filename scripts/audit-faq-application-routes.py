#!/usr/bin/env python3
"""Проверяет прямые маршруты со страницы FAQ в онлайн-заявку."""
...
class PageParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag.lower() == "article" and "question-card" in classes:
...
def validate_source(path: Path) -> int:
    ...
    if 'href="{{ \'/online-zayavka/\' | relative_url }}"' in text:
        fail(path, "На странице осталась прямая ссылка на анкету без источника и сценария")
        errors += 1

    if text.count('class="question-card"') != 13:
        fail(path, "Исходник должен сохранять 13 карточек вопросов")
        errors += 1
...
def validate_built(path: Path) -> int:
    parser = PageParser()
    built_text = path.read_text(encoding="utf-8-sig", errors="ignore")
    ...
    if found_labels != ROUTE_LABELS:
        fail(path, "Подписи прямых CTA не совпадают с ожидаемыми")
        errors += 1
    ...
    if parser.question_cards != 13:
        fail(path, f"В собранной странице должно быть 13 карточек вопросов, найдено: {parser.question_cards}")
        errors += 1
    ...
    if phone_links < 2:
        ...
        errors += 1
    if "data-copy-phone" not in built_text:
        fail(path, "На собранной странице отсутствует кнопка MAX")
        errors += 1
    if "https://vk.com/tatyanasterlikova" not in built_text:
        fail(path, "На собранной странице отсутствует ссылка ВКонтакте")
        errors += 1
...
def main() -> int:
    ...
    built_file = site_dir / "faq" / "index.html"
    errors = 0

    if not SOURCE_FILE.is_file():
        fail(SOURCE_FILE, "Исходная страница FAQ отсутствует")
        errors += 1
    else:
        errors += validate_source(SOURCE_FILE)

    if not built_file.is_file():
        fail(built_file, "Собранная страница FAQ отсутствует")
        errors += 1
    else:
        errors += validate_built(built_file)

    if errors:
        print(f"Аудит прямых заявок со страницы FAQ завершён с ошибками: {errors}")
        return 1

    print(
        "Маршруты FAQ подтверждены: два прямых CTA, источник /faq/, "
        "единый сценарий, 13 вопросов и сохранённые контакты"
    )
    return 0
